   ...
   stage ('SAST - SonarQube') {
	...
	stage ('DAST - OWASP ZAP') {
           ...
           sshagent(['dast-server']) {
                sh 'ssh -o  StrictHostKeyChecking=no ubuntu@3.111.169.114 "sudo docker run --rm -v /home/ubuntu:/zap/wrk/:rw -t zaproxy/zap-stable zap-full-scan.py -t http://3.110.210.81:8081/WebGoat -x zap_report || true" '
